...
class ExamsSummativeOrders extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        return view('livewire.exams-summative-orders', [
        ...
        if ($examOrder) {
            if ($examOrder->status == 0 || $examOrder->status == -1) {
                if (auth()->user()->current_role == 'مشرف' ||
                    ...
                    $this->process_notification($arr_external_user_ids, 1, $examOrder->student->user->name, $examOrder->quranPart->QuranSummativePartName());
                    $this->dispatchBrowserEvent('alert',
                        ['type' => 'success', 'message' => 'تمت عملية قبول طلب اختبار التجميعي بنجاح.']);
                }
                ...
                if ($this->teacher_id == $this->tester_id) {
                    ...
                    if (auth()->user()->current_role == 'مشرف الإختبارات' ||
                        ...
                        $this->process_notification($arr_external_user_ids, 2, $examOrder->student->user->name, $examOrder->quranPart->QuranSummativePartName());

                        // push notification to tester
                        if (auth()->user()->current_role == 'أمير المركز') {
                            $message = "لقد قام أمير المركز بتعيينك مختبر طلب اختبار " . $examOrder->quranPart->QuranSummativePartName() . " للطالب: " . $examOrder->student->user->name . " يرجى مراجعة الموعد المحدد للطلب ...";
                        } else if (auth()->user()->current_role == 'مشرف الإختبارات') {
                            $message = "لقد قام مشرف الإختبارات بتعيينك مختبر طلب اختبار " . $examOrder->quranPart->QuranSummativePartName() . " للطالب: " . $examOrder->student->user->name . " يرجى مراجعة الموعد المحدد للطلب ...";
                        }
                        $response = $this->push_notifications([$examOrder->tester_id], $message);
                        $return["allresponses"] = $response;
                        $return = json_encode($return);
                        $this->dispatchBrowserEvent('alert',
                            ['type' => 'success', 'message' => 'تمت عملية اعتماد طلب اختبار التجميعي بنجاح.']);
    ...
    public function process_notification($arr_external_user_ids, $status, $student_name, $part_name)
    {
        if ($arr_external_user_ids != null && $status != null) {
            ...
            if (auth()->user()->current_role == 'أمير المركز') {
                if ($status == 1) {
                    $message = "لقد قام أمير المركز بقبول طلب اختبار التجميعي " . $part_name . " للطالب: " . $student_name . " يرجى مراجعة طلب الاختبار ...";
                } else if ($status == -1) {
                    $message = "لقد قام أمير المركز برفض طلب اختبار التجميعي " . $part_name . " للطالب: " . $student_name . " يرجى مراجعة طلب الاختبار ...";
                } else if ($status == 2) {
                    $message = "لقد قام أمير المركز بحجز طلب اختبار التجميعي " . $part_name . " للطالب: " . $student_name . " يرجى مراجعة الموعد المحدد للطلب ...";
                } elseif ($status == -2) {
                    $message = "لقد قام أمير المركز برفض حجز طلب اختبار التجميعي " . $part_name . " للطالب: " . $student_name . " يرجى مراجعة طلب الاختبار ...";
                }
            } else if (auth()->user()->current_role == 'مشرف الإختبارات') {
                if ($status == 2) {
                    $message = "لقد قام مشرف الإختبارات بحجز طلب اختبار التجميعي " . $part_name . " للطالب: " . $student_name . " يرجى مراجعة الموعد المحدد للطلب ...";
                } elseif ($status == -2) {
                    $message = "لقد قام مشرف الإختبارات برفض حجز طلب اختبار التجميعي " . $part_name . " للطالب: " . $student_name . " يرجى مراجعة طلب الاختبار ...";
                }
            } else if (auth()->user()->current_role == 'مشرف') {
                if ($status == 1) {
                    $message = "لقد قام مشرف المرحلة بقبول طلب اختبار التجميعي " . $part_name . " للطالب: " . $student_name . " يرجى مراجعة طلب الاختبار ...";
                } else if ($status == -1) {
                    $message = "لقد قام مشرف المرحلة برفض طلب اختبار التجميعي " . $part_name . " للطالب: " . $student_name . " يرجى مراجعة طلب الاختبار ...";
                }
            }
    ...
    public function push_notifications($arr_external_user_ids, $message)
    {
        $fields = array(
            ...
            'data' => array("foo" => "bar"),
            'headings' => array(
                "en" => 'حالة طلب اختبار التجميعي',
                "ar" => 'حالة طلب اختبار التجميعي',
            ),
        ...
        if ($examOrder) {
            if ($examOrder->status == 0) {
                ...
                if (auth()->user()->current_role == 'مشرف' ||
                    ...
                    $this->dispatchBrowserEvent('alert',
                        ['type' => 'error', 'message' => 'تمت عملية رفض طلب اختبار التجميعي بنجاح.']);
                    $this->clearForm();
                    $this->process_notification([$examOrder->teacher_id], -1, $examOrder->student->user->name, $examOrder->quranPart->QuranSummativePartName());
                }
                ...
                if (auth()->user()->current_role == 'مشرف الإختبارات' ||
                    ...
                    $this->dispatchBrowserEvent('alert',
                        ['type' => 'error', 'message' => 'تمت عملية رفض طلب اختبار التجميعي بنجاح.']);
                    $this->clearForm();
                    $this->process_notification([$examOrder->teacher_id], -2, $examOrder->student->user->name, $examOrder->quranPart->QuranSummativePartName());
                }
    ...
    {
        $exam_order = ExamSummativeOrder::find($id);
        if ($exam_order->status == 0 ||
            ...
            $this->dispatchBrowserEvent('alert',
                ['type' => 'error', 'message' => 'تم حذف طلب اختبار التجميعي بنجاح.']);
        }
    }
}
