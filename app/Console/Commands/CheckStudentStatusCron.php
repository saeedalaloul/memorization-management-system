    ...
    public function handle()
    {
        ...
        foreach ($students as $student)
            if ($student->warning_expiry_date == null) {
                array_push($studentWarningsAbsence, [
                    'id' => Str::uuid()->toString(),
                    'student_id' => $student->id,
                    'reason' => StudentWarning::ABSENCE_REASON,
                    'details' => json_encode(['number_times' => $student->number_times]),
                    'created_at' => Carbon::now(),
                    'updated_at' => Carbon::now(),
                ]);
            } else {
                if (intval($student->attendance_count / $student->number_times) > $student->warning_count) {
                    array_push($studentWarningsAbsence, [
                        'id' => Str::uuid()->toString(),
                        'student_id' => $student->id,
                        'reason' => StudentWarning::ABSENCE_REASON,
                ...
                foreach ($studentWarningsAbsence as $item) {
                    ...
                    if ($student_warning) {
                        ...
                        $teacher_user?->notify(new NewStudentWarningForTeacherNotify([
                            ...
                            'reason' => $item['reason'],
                            'number_times' => json_decode($item['details'])->number_times,
                        ]));
                        $title = "إنذار طالب جديد";
                        $message = "";
                        if ($item['reason'] == StudentWarning::ABSENCE_REASON) {
                            $message = "لقد تم إعطاء الطالب: " . $student_name . " إنذار نهائي بسبب غيابه المتكرر لمدة " . json_decode($item['details'])->number_times . " أيام,راجع مشرف المرحلة حتى لا يتم تجميد عمليات الطالب!";
                        }
                        $this->push_notification($message, $title, [$teacher_user?->user_fcm_token->device_token ?? null]);
                    }
                }
            } catch (\Exception $e) {
                DB::rollback();
...
/////////////////////////////////////////////////////////////
        ...
        foreach ($students as $student)
            if ($student->block_expiry_date == null) {
                array_push($studentBlocksAbsence, [
                    'id' => Str::uuid()->toString(),
                    'student_id' => $student->id,
                ...
                if (intval($student->attendance_count / $student->number_times) > $student->block_count) {
                    array_push($studentBlocksAbsence, [
                        'id' => Str::uuid()->toString(),
                        'student_id' => $student->id,
                        'reason' => StudentBlock::ABSENCE_REASON,
                ...
                foreach ($studentBlocksAbsence as $item) {
                    ...
                    if ($student_block) {
                        ...
                        if ($item['reason'] == StudentBlock::ABSENCE_REASON) {
                            $message = "لقد تم حظر الطالب: " . $student_name . " بسبب غيابه المتكرر لمدة " . json_decode($item['details'])->number_times . " أيام,راجع أمير المركز!";
                        }
                        $this->push_notification($message, $title, [$teacher_user?->user_fcm_token->device_token ?? null]);
                    }
                }
...
//////////////////////////////////////////////////////////////////////////////////////////
        ...
        foreach ($students as $student)
            if ($student->warning_expiry_date == null) {
                array_push($studentWarningsLate, [
                    'id' => Str::uuid()->toString(),
                    'student_id' => $student->id,
                ...
                foreach ($studentWarningsLate as $item) {
                    ...
                    if ($student_warning) {
                        ...
                        if ($item['reason'] == StudentWarning::LATE_REASON) {
                            $message = "لقد تم إعطاء الطالب: " . $student_name . " إنذار نهائي بسبب تأخره المتكرر لمدة " . json_decode($item['details'])->number_times . " أيام,راجع مشرف المرحلة حتى لا يتم تجميد عمليات الطالب!";
                        }
                        $this->push_notification($message, $title, [$teacher_user?->user_fcm_token->device_token ?? null]);
                    }
                }
            ...
            try {
                ...
                foreach ($studentBlocksLate as $item) {
                    ...
                    if ($student_block) {
                        ...
                        $message = "";
                        if ($item['reason'] == StudentBlock::ABSENCE_REASON) {
                            $message = "لقد تم حظر الطالب: " . $student_name . " بسبب تأخره المتكرر لمدة " . json_decode($item['details'])->number_times . " أيام,راجع أمير المركز!";
                        }
                        $this->push_notification($message, $title, [$teacher_user?->user_fcm_token->device_token ?? null]);
                    }
                }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        foreach ($students as $student)
            if ($student->warning_expiry_date == null) {
                array_push($studentWarningsMemorize, [
                    'id' => Str::uuid()->toString(),
                    'student_id' => $student->id,
                    'reason' => StudentWarning::MEMORIZE_REASON,
                    'details' => json_encode(['number_times' => $student->number_times, 'number_pages' => $student->quantity]),
                    'created_at' => Carbon::now(),
                    'updated_at' => Carbon::now(),
                ]);
            } else {
                if (intval($student->daily_memorization_count / $student->number_times) > $student->warning_count) {
                    array_push($studentWarningsMemorize, [
                        'id' => Str::uuid()->toString(),
                        'student_id' => $student->id,
                        'reason' => StudentWarning::MEMORIZE_REASON,
                ...
                foreach ($studentWarningsMemorize as $item) {
                    ...
                    if ($student_warning) {
                        ...
                        $teacher_user?->notify(new NewStudentWarningForTeacherNotify([
                            ...
                            'reason' => $item['reason'],
                            'number_times' => json_decode($item['details'])->number_times,
                            'number_pages' => json_decode($item['details'])->number_pages,
                        ]));
                        $title = "إنذار طالب جديد";
                        $message = "";
                        if ($item['reason'] == StudentWarning::MEMORIZE_REASON) {
                            $message = "لقد تم إعطاء الطالب: " . $student_name . " إنذار نهائي بسبب تسميعه المتكرر أقل من " .json_decode($item['details'])->number_pages . " صفحة لمدة " . json_decode($item['details'])->number_times . " أيام,راجع مشرف المرحلة حتى لا يتم تجميد عمليات الطالب!";
                        }
                        $this->push_notification($message, $title, [$teacher_user?->user_fcm_token->device_token ?? null]);
                    }
                }
        ...
        foreach ($students as $student)
            if ($student->block_expiry_date == null) {
                array_push($studentBlocksMemorize, [
                    'id' => Str::uuid()->toString(),
                    'student_id' => $student->id,
                    'reason' => StudentWarning::MEMORIZE_REASON,
                    'details' => json_encode(['number_times' => $student->number_times, 'number_pages' => $student->quantity]),
                ...
                if (intval($student->daily_memorization_count / $student->number_times) > $student->block_count) {
                    array_push($studentBlocksMemorize, [
                        'id' => Str::uuid()->toString(),
                        'student_id' => $student->id,
                        'reason' => StudentWarning::MEMORIZE_REASON,
            ...
            try {
                ...
                foreach ($studentBlocksMemorize as $item) {
                    ...
                    if ($student_block) {
                        ...
                        $message = "";
                        if ($item['reason'] == StudentBlock::ABSENCE_REASON) {
                            $message = "لقد تم حظر الطالب: " . $student_name . " بسبب تسميعه المتكرر أقل من " . json_decode($item['details'])->number_pages . " صفحة لمدة " . json_decode($item['details'])->number_times . " أيام,راجع أمير المركز!";
                        }
                        $this->push_notification($message, $title, [$teacher_user?->user_fcm_token->device_token ?? null]);
                    }
                }
        ...
        foreach ($students as $student)
            if ($student->warning_expiry_date == null) {
                array_push($studentWarningsNotMemorize, [
                    'id' => Str::uuid()->toString(),
                    'student_id' => $student->id,
                ...
                if (intval($student->memorization_count / $student->number_times) > $student->warning_count) {
                    array_push($studentWarningsNotMemorize, [
                        'id' => Str::uuid()->toString(),
                        'student_id' => $student->id,
                        'reason' => StudentWarning::DID_NOT_MEMORIZE_REASON,
                ...
                foreach ($studentWarningsNotMemorize as $item) {
                    ...
                    if ($student_warning) {
                        ...
                        if ($item['reason'] == StudentWarning::DID_NOT_MEMORIZE_REASON) {
                            $message = "لقد تم إعطاء الطالب: " . $student_name . " إنذار نهائي بسبب عدم الحفظ المتكرر لمدة " . json_decode($item['details'])->number_times . " أيام,راجع مشرف المرحلة حتى لا يتم تجميد عمليات الطالب!";
                        }
                        $this->push_notification($message, $title, [$teacher_user?->user_fcm_token->device_token ?? null]);
                    }
                }
        ...
        foreach ($students as $student)
            if ($student->block_expiry_date == null) {
                array_push($studentBlocksNotMemorize, [
                    'id' => Str::uuid()->toString(),
                    'student_id' => $student->id,
                ...
                if (intval($student->memorization_count / $student->number_times) > $student->block_count) {
                    array_push($studentBlocksNotMemorize, [
                        'id' => Str::uuid()->toString(),
                        'student_id' => $student->id,
                        'reason' => StudentBlock::DID_NOT_MEMORIZE_REASON,
            ...
            try {
                ...
                foreach ($studentBlocksNotMemorize as $item) {
                    ...
                    if ($student_block) {
                        ...
                        $message = "";
                        if ($item['reason'] == StudentBlock::ABSENCE_REASON) {
                            $message = "لقد تم حظر الطالب: " . $student_name . " بسبب عدم الحفظ المتكرر لمدة " . json_decode($item['details'])->number_times . " أيام,راجع أمير المركز!";
                        }
                        $this->push_notification($message, $title, [$teacher_user?->user_fcm_token->device_token ?? null]);
                    }
                }
