        ...
        <div class="modal-content">
            ...
            <div class="modal-body p-20">
                <form>
                    <div class="row">
                        <div class="col-md-6">
                            <label class="control-label">اسم المختبر</label>
                            <input type="text" wire:model.defer="tester_name" readonly class="form-control">
                            @error('tester_name')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            <label class="control-label">عدد اختبارات المختبر</label>
                            <input type="text" wire:model.defer="exams_count" readonly class="form-control">
                            @error('exams_count')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            ...
                            <select class="form-control form-white" wire:model.defer="oversight_member_id"
                                ...
                                <option selected value="{{0}}">اختر المراقب...</option>
                                @foreach($oversight_members as $oversight_member)
                                    @if($oversight_member->id != $tester_id)
